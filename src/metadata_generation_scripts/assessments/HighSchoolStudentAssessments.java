// class HighSchoolStudentAssessments
//
...
// stored in a HighSchoolStudentAssessments class, and printed out in a specified order and
// format (See ParseHighSchoolStudentAssessments.java for the format).
//
// Both Concert Band and Symphonic Band students have the same assessments, so
// they are both reperesented by this HighSchoolStudentAssessments class.
//
// Authored by Chris Laguna
public class HighSchoolStudentAssessments {
  ...
  private float technical_etude_artistry;
  ...
  private float scales_gb;
  private float scales_b;
  private float scales_e;
  private float scales_a;
  private float scales_d;
  ...
  public HighSchoolStudentAssessments(int s_id) {
    student_id = s_id;
  ...
  public void normalizeFromMax(MaxAssessments maxes) {
    ...
    if (this.scales_gb != -1.f) {
      this.scales_gb /= maxes.scales_gb;
    }    
    if (this.scales_b != -1.f) {
      this.scales_b /= maxes.scales_b;
    }
    if (this.scales_e != -1.f) {
      this.scales_e /= maxes.scales_e;
    }
    if (this.scales_a != -1.f) {
      this.scales_a /= maxes.scales_a;
    }    
    if (this.scales_d != -1.f) {
      this.scales_d /= maxes.scales_d;
    }
    ...
    if (this.technical_etude_artistry != -1.f) {
      this.technical_etude_artistry /= maxes.technical_etude_artistry;
    }
  ...
  // (See ParseHighSchoolStudentAssessments for information on the format).
  public String toString() {
    return 
        ...
        "-1.0" +
        "\t" + (this.technical_etude_artistry < 0 ? "-1.0" : String.format("%.2f", this.technical_etude_artistry)) +
