...
def max_scores_func(year, band):
    ...
    instrument_max_scores = [line for line in max_scores_file if
                             line[0] == bands_dict[band] and line[1][:4] == year and line[5] == 'NULL']
    percussion_max_scores = [line for line in max_scores_file if
                             line[0] == bands_dict[band] and line[1][:4] == year and line[5] == '1']
...
class Student:
    ...
    def add_assessment(self, assessment, v):
        if self.instrument.lower() == 'percussion':
            try:
                max_score = PERCUSSION_MAX_SCORES[standardize(assessment)]
            except:
                print(
                    f'could not find max_scores, {self.year}, {self.band}, {self.instrument}, {standardize(assessment)}'
                )
                max_score = 1
        else:
            try:
                max_score = INSTRUMENT_MAX_SCORES[standardize(assessment)]
            except:
                print(
                    f'could not find max_scores, {self.year}, {self.band}, {self.instrument}, {standardize(assessment)}'
                )
        ...
        val = v / max_score
        if val > 1:
            print(f'Score exceeds 1: {self.student_id}, {standardize(assessment)}')
            BAD_SCORES.append((self.student_id, standardize(assessment), v, max_score))
    ...
    for folder in os.listdir(os.getcwd()):
        ...
        if folder.startswith('FBA'):
            ...
            for sub_folder in os.listdir(os.getcwd()):
                if sub_folder in ['middleschool', 'symphonicband', 'concertband']:
                    ...
                    unique_student_ids = df.Student.unique()
                    for sid in unique_student_ids:
                        se = df.loc[df.Student == sid]
                        st = Student(
                            int(se.Student.loc[se.index.values[0]]),
                            df.year,
                            df.band,
                            se.Instrument.loc[se.index.values[0]]
                        )
                        for i in se.index.values:
                            st.add_assessment(
                                extract_alpha(se.ScoreGroup.loc[i] + se.Category.loc[i]), se.Score.loc[i]
                            )

                        if se.Instrument.loc[se.index.values[0]] == 'Percussion':
                            percussion_students.append(st)
                        else:
                            students.append(st)
